...
omg = 1;
    ...
    for i=1:1000
        ...
        dxd= -omg*A*sin(omg*t);
        dyd= omg*A*cos(omg*t);
        %dzd= (omg*A/2)*cosd(omg*t);
        dzd = 0;
        dfid = 0;
        d2xd= -(omg^2)*A*cos(omg*t);
        d2yd= -(omg^2)*A*sin(omg*t);
        %d2zd= -(omg^2)*(A/2)*sind(omg*t);
        d2zd =0;
        d2fid = 0;
    %         if traj_opt == 1
    %             to = t;
...
%         set(gui.posAtual_yaw,'string',fi*180/pi);
    ...
    %     sum_qtil = sum_qtil + norm(q_til(1:3))
    %     sum_psitil = sum_psitil + norm(q_til(4))
    %     sum_v = sum_v + norm(v)

    %     sum_qtil_v(i) = sum_qtil;


    end
        jk = jk+1;
        normEPM_til(jk) =  norm(q_til_v(1:3,i));
        normEPsiM_til(jk) = norm(q_til_v(4,i));
        normU(jk) = norm(v_v(i));
        u_barra_med(jk,:,:) = abs(v_v);
end
    ...
    EPM = sum(normEPM_til)/jk
    EPsiM = sum(normEPsiM_til)/jk
    u_norm_media = sum(normU)/jk
